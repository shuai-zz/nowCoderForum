...
import org.example.nowcoder.entity.dto.CreatePostRequest;
import org.example.nowcoder.entity.vo.CommentVO;
import org.example.nowcoder.entity.vo.PostDetailVO;
import org.example.nowcoder.entity.vo.PostListItemVO;
import org.example.nowcoder.entity.vo.ReplyVO;
...
public class PostController {
    ...
    public Result<PageResult<PostListItemVO>> list(
        ...
        if (page.getList() != null) {
            for (DiscussPost post : page.getList()) {
                UserVO author = UserVO.from(userService.findUserById(post.getUserId()));
    ...
    private CommentVO buildCommentVO(Comment c) {
        UserVO author = UserVO.from(userService.findUserById(c.getUserId()));
    ...
    private ReplyVO buildReplyVO(Comment r) {
        UserVO rAuthor = UserVO.from(userService.findUserById(r.getUserId()));
        UserVO target = r.getTargetId() == 0
                ? null
                : UserVO.from(userService.findUserById(r.getTargetId()));
